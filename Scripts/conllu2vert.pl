...
#Insert <g/> and <name> and <headword>
sub finalize {
    my @lines = @_;
    my $ner = 'O';
    my $prev_ner = 'O';
	...
	my ($token, $lemma, $XPoS, $UPoS, $UFeats, $i,
        ...
        if ($headword) {
            ...
            elsif ($headword =~ m|^\Q$token\E|) {
                $headword =~ s|^\Q$token\E\s*||;
                $headword = '666' unless $headword;
                push(@out, "<headword>") unless @out;
            }
            else {
                die "FATAL: mismatch in headword $headword and token $token\n"
            }
        }
